...
# goDiamond supports: mod (2v), adm, com, prf, stm (stb)
# Mapping: mod -> 2v, adm -> adm, com -> com, prf -> prf, stm -> stm
...
for problem in mod adm com prf stm; do
    problem_short="${PROBLEM_MAP[$problem]}"
    fast_file="${RESULTS_DIR}/${problem_short}_fast.txt"
    output_file="${OUTPUT_DIR}/go_diamond_${problem_short}.txt"
    ...
    echo "Recording goDiamond results for $problem..."
    echo "=== goDiamond $problem ===" > "$output_file"
    ...
    while IFS= read -r instance; do
        ...
        ./benchmarks/run_goDiamond.sh ${ARG_MAP[$problem]} "$instance_file" >> "$output_file" 2>&1 || echo "Error running $instance" >> "$output_file"
...
echo "All goDiamond recordings completed!"
