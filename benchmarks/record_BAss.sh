...
# BAss supports: stable, two-valued, preferred, complete, admissible
# Mapping: stable -> stm, two-valued -> 2v, preferred -> prf, complete -> com, admissible -> adm
...
for problem in stable "two-valued" preferred complete admissible; do
    ...
    output_file="${OUTPUT_DIR}/bass_${problem_short}.txt"
    ...
    echo "Recording BAss results for $problem..."
    echo "=== BAss $problem ===" > "$output_file"
    ...
    while IFS= read -r instance; do
        ...
        ./benchmarks/run_BAss.sh ${ARG_MAP[$problem]} "$instance_file" >> "$output_file" 2>&1 || echo "Error running $instance" >> "$output_file"
...
echo "All BAss recordings completed!"
